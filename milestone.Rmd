...
```{r Coverage plots - Bigram, echo=FALSE} 

plot(bigrams$cumpct,ylab="Coverage (%)",xlab="Term Count",main="Coverage for 2-gram Terms")
abline(h=50)
abline(h=80)
abline(h=90)
...
The number of bigrams to achieve 50% coverage is ```r length(grams[which(grams$cumpct<=50),c("grams")])```; 
80%: ```r length(grams[which(grams$cumpct<=80),c("grams")])```; and 90%: 
```r length(grams[which(grams$cumpct<=90),c("grams")])```.


```{r Coverage plots - Trigram, echo=FALSE} 

plot(trigrams$cumpct,ylab="Coverage (%)",xlab="Term Count",main="Coverage for 3-gram Terms")
abline(h=50)
abline(h=80)
abline(h=90)
...
My plan for the remaining time for this project is to:
...
- Apply clustering to find word associations. As I understand it, I can use clustering to predict relevant words
- Create the prediction algorithm -- How? I have yet to study this 
